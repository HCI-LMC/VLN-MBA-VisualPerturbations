...
from model_ops import create_transformer_encoder
from model_ops import extend_neg_masks, gen_seq_masks, pad_tensors_wgrad
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.local2_encoder = LocalVPEncoder(config)
        self.global_encoder = GlobalMapEncoder(config)
        self.global2_encoder = GlobalMapEncoder(config)

        self.global_sap_head = ClsPrediction(self.config.hidden_size)
        self.global2_sap_head = ClsPrediction(self.config.hidden_size)
        self.local_sap_head = ClsPrediction(self.config.hidden_size)
        self.local2_sap_head = ClsPrediction(self.config.hidden_size)
    ...
    #         self, view_depth_fts, obj_img_fts, loc_fts, nav_types, view_lens, obj_lens
    # ):
    #     device = view_depth_fts.device
    #     has_obj = obj_img_fts is not None
    #
    #     view_img_embeds = self.depth_embeddings.img_layer_norm(
    #         self.img_embeddings.depth_linear(view_depth_fts)
    #     )
    #     if has_obj:
    #         if self.depth_embeddings.obj_linear is None:
    #             obj_img_embeds = self.depth_embeddings.img_layer_norm(
    #                 self.img_embeddings.img_linear(obj_img_fts)
    #             )
    #         else:
    #             obj_img_embeds = self.depth_embeddings.obj_layer_norm(
    #                 self.depth_embeddings.obj_linear(obj_img_fts)
    #             )
    #         img_embeds = []
    #         for view_embed, obj_embed, view_len, obj_len in zip(
    #                 view_img_embeds, obj_img_embeds, view_lens, obj_lens
    ...
    #
    #     pano_embeds = img_embeds + \
    #                   self.depth_embeddings.loc_layer_norm(self.depth_embeddings.loc_linear(loc_fts)) + \
    #                   self.depth_embeddings.nav_type_embedding(nav_types) + \
    #                   self.embeddings.token_type_embeddings(torch.ones(1, 1).long().to(device))
    #     pano_embeds = self.depth_embeddings.layer_norm(pano_embeds)
    #     pano_embeds = self.depth_embeddings.dropout(pano_embeds)
    #
    #     pano_masks = gen_seq_masks(pano_lens)
    #     if self.depth_embeddings.pano_encoder is not None:
    #         pano_embeds = self.depth_embeddings.pano_encoder(
    #             pano_embeds, src_key_padding_mask=pano_masks.logical_not()
    #         )
    #     return pano_embeds, pano_masks
    ...
    def forward_depth_panorama_per_step(
        ...
        view_img_embeds = self.img_embeddings.img_layer_norm(
            self.img_embeddings.depth_linear(view_img_fts)
        )
        if has_obj:
            if self.img_embeddings.obj_linear is None:
                obj_img_embeds = self.img_embeddings.img_layer_norm(
    ...
    def forward_navigation_per_step(
            self, txt_embeds, txt_masks, gmap_img_embeds, gmap_depth_embeds, gmap_step_ids, gmap_pos_fts,
            gmap_masks, gmap_pair_dists, gmap_visited_masks, gmap_vpids,
            vp_img_embeds, vp_depth_embeds, vp_pos_fts, vp_masks, vp_nav_masks, vp_obj_masks, vp_cand_vpids,
    ):
        ...
        gmap_img_embeds_neg = gmap_img_embeds[index]
        # gmap_masks = gmap_masks[index]
        c = 0.5
        gmap_img_embeds_neg = (1 - c) * gmap_img_embeds + c * gmap_img_embeds_neg
        gmap2_embeds = gmap_img_embeds_neg + \
                      self.global2_encoder.gmap_step_embeddings(gmap_step_ids) + \
                      self.global2_encoder.gmap_pos_embeddings(gmap_pos_fts)

        gmap2_embeds = self.global2_encoder.encoder(
        ...
        index = torch.randperm(batch_size)
        vp_img_embeds_neg = vp_img_embeds[index]
        # gmap_masks = gmap_masks[index]
        c = 0.5
        vp_img_embeds_neg = (1 - c) * vp_img_embeds + c * vp_img_embeds_neg

        vp2_embeds = vp_img_embeds_neg + self.local_encoder.vp_pos_embeddings(vp_pos_fts)
        vp2_embeds = self.local2_encoder.encoder(txt_embeds, txt_masks, vp2_embeds, vp_masks)
        ...
        if self.sap_fuse_linear is None:
            ...
            fuse_weights = torch.sigmoid(self.sap_fuse_linear(
                torch.cat([gmap_embeds[:, 0],gmap2_embeds[:, 0], vp2_embeds[:, 0], vp_embeds[:, 0]], 1)
            ))
            fw = F.softmax(fuse_weights, 1)
        fw_global = fw[:, 0].unsqueeze(1)
        fw_global2 = fw[:, 1].unsqueeze(1)
        fw_local2 = fw[:, 2].unsqueeze(1)
        fw_local = fw[:, 3].unsqueeze(1)
        ...
        global2_logits = self.global2_sap_head(gmap2_embeds).squeeze(2) * fw_global2
        ...
        local2_logits = self.local2_sap_head(vp2_embeds).squeeze(2) * fw_local2
        local2_logits.masked_fill_(vp_nav_masks.logical_not(), -float('inf'))

        global_fuse_logits = global_logits + global2_logits

        local_logits = local_logits + local2_logits
    ...
    def forward(self, mode, batch, **kwargs):
        if mode == 'language':
            ...
            return self.forward_navigation_per_step(
                batch['txt_embeds'], batch['txt_masks'], batch['gmap_img_embeds'], batch['gmap_depth_embeds'],
                batch['gmap_step_ids'], batch['gmap_pos_fts'], batch['gmap_masks'],
                batch['gmap_pair_dists'], batch['gmap_visited_masks'], batch['gmap_vpids'],
                batch['vp_img_embeds'], batch['vp_depth_embeds'], batch['vp_pos_fts'], batch['vp_masks'],
