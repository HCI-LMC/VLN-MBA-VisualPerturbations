...
from data import load_nav_graphs, new_simulator
from data import angle_feature, get_all_point_angle_feature
...
class EnvBatch(object):
    ...
    def __init__(self, connectivity_dir, scan_data_dir=None, feat_db=None,dfeat_db=None, batch_size=100):
        """
    ...
    def getStates(self):
        ...
        for i, sim in enumerate(self.sims):
            ...
            feature_depth_states.append((feature,dfeature, state))

        return feature_depth_states
...
class ReverieObjectNavBatch(object):
    ...
    def __init__(
        self, view_db, dview_db, obj_db, instr_data, connectivity_dir, obj2vps,
        ...
        self.env = EnvBatch(connectivity_dir, feat_db=view_db, dfeat_db=dview_db, batch_size=batch_size)
        self.obj_db = obj_db
    ...
    def _get_obs(self):
        obs = []
        for i, (feature, dfeature, state) in enumerate(self.env.getStates()):
            item = self.batch[i]
            base_view_id = state.viewIndex

            # Full features
            candidate = self.make_candidate(feature, dfeature,state.scanId, state.location.viewpointId, state.viewIndex)
