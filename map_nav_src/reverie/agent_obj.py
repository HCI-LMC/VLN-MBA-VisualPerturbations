...
from ops import pad_tensors, gen_seq_masks
from torch.nn.utils.rnn import pad_sequence

from agent_base import Seq2SeqAgent

from graph_utils import GraphMap
from model import VLNBert, Critic
from model_ops import pad_tensors_wgrad
...
class GMapObjectNavAgent(Seq2SeqAgent):
    ...
    def _panorama_feature_variable(self, obs):
        ...
        batch_view_img_fts, batch_view_depth_fts, batch_obj_img_fts, batch_loc_fts, batch_nav_types = [], [], [], [], []
        batch_view_lens, batch_obj_lens = [], []
        batch_cand_vpids, batch_objids = [], []

        for i, ob in enumerate(obs):
            view_img_fts, view_depth_fts, view_ang_fts, nav_types, cand_vpids = [], [], [], [], []
            # cand views
            used_viewidxs = set()
    ...
    def _nav_vp_variable(self, obs, gmaps, pano_embeds, pano_depth_embeds, cand_vpids, view_lens, obj_lens, nav_types):
        batch_size = len(obs)
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        for t in range(self.args.max_action_len):
            ...
            pano_depth_embeds, pano_masks = self.vln_bert('depth_panorama', pano_inputs)  # here 补充view的view_depth_fts
            avg_pano_depth_embeds = torch.sum(pano_depth_embeds * pano_masks.unsqueeze(2), 1) / \
                                    torch.sum(pano_masks, 1, keepdim=True)  # here--------------------------
            ...
            # print('gmap_depth_embeds',nav_inputs['gmap_depth_embeds'].size())
            # print('gmap_img_embeds',nav_inputs['gmap_img_embeds'].size())

            nav_inputs.update(
                self._nav_vp_variable(
                    obs, gmaps, pano_embeds, pano_depth_embeds, pano_inputs['cand_vpids'],
        ...
        if train_ml is not None:
            ...
            neg_loss = neg_loss * train_ml / batch_size
            self.loss += ml_loss
            self.loss += og_loss
            self.loss += neg_loss
